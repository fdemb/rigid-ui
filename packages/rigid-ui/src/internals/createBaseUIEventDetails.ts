 ...
 * `reference/base-ui/packages/react/src/internals/createBaseUIEventDetails.ts`. Framework-free;
 * only the EMPTY_OBJECT dependency is dropped.
 */
...
/**
 * Details of custom change events emitted by Rigid UI components.
 */
...
/**
 * Details of custom generic events emitted by Rigid UI components (no cancel semantics).
 */
...
/**
 * Creates a Base UI event details object with the given reason and utilities for preventing
 * Rigid UI's internal event handling. The cancellation flags are closure state so handlers can
 * mutate them synchronously while components observe them after the callback returns.
 */
...
>(
  ...
  const custom = customProperties ?? ({} as CustomProperties);
