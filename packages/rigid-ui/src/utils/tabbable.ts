...
export function ownerDocument(reference: Element | Node | null | undefined): Document {
  return (reference?.ownerDocument ?? document) as Document;
}

export function ownerWindow(reference: Element | Node | null | undefined): typeof window {
  return ownerDocument(reference).defaultView ?? window;
}
...
export function isElementVisible(
  ...
  if (!element || !element.isConnected || !styles) {
    return false;
  }
...
function getComposedChildren(container: ParentNode): Element[] {
  if (container instanceof HTMLElement && getNodeName(container) === "slot") {
