...
const markerName = "data-rigid-ui-inert";
...
function unwrapHost(node: Node | null): Element | null {
  ...
  return unwrapHost(node.parentNode);
}
...
export function markOthers(avoidElements: Element[], options: MarkOthersOptions = {}): Undo {
  ...
  const doc = avoidElements[0]?.ownerDocument ?? document;
  return applyAttributeToOthers(avoidElements, doc.body, ariaHidden, inert, { mark });
}
